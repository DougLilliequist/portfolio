...
void main() {
    ...
    vec3 flowVec = texture2D(flowField, uv).xyz;

    vec3 curl = curlNoise(vec3(flowVec.xy * 1.22, animTime * 0.000015));

    curl += animTime * 0.1;

    vec3 noise = curl * 0.5 + 0.5;
    
    noise *= 3.0;
    vec3 dir = mousePos - noise;

    dir = normalize(dir);

    // float flowDir = range(snoise((flowVec * 0.1) + animTime * 0.54), -1.0, 1.0, -10.0, 10.0);

    // dir *= flowDir;

    // flowVec += flowDir;

    // vec3 flowDir = random(uv) * flowVec;
    // vec3 flowDir = vec3(resolution.x * 0.5, 0.0, 0.0) - flowVec;
